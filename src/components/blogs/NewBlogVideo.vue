...
const updateBlog = async () => {
  ...
  const videoRef = storageRef(
    storage,
    "videos/" + props.blogId + "/" + timestamp
  );
  await uploadBytes(videoRef, blogVideo.value).then(async () => {
    ...
    await updateDoc(blogRef, updatedBlogData).then(() => {
      emit("video-updated", blogVideoUrl);
      loading.value = false;
    });
  });
};
...
<template>
  <div>
    <form @submit.prevent="updateBlog">
      <div v-if="videoUrl" class="avatar flex justify-center">
        <video ref="VideoEditor" controls autoplay class="rounded-xl w-[30rem]">
